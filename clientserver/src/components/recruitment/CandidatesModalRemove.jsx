...
export default function CandidatesModalRemove({ data: data_, close, show }) {
    const { messenger, data } = data_;
    const ticket = JSON.parse(localStorage.getItem('data'));

    const initialState = {
        spin: false,
        name: data.name,
        from: messenger.username,
        subject: '',
        to: data.email,
        cc: '',
        isGoogleSetup: messenger.isGoogleSetup,
    ...
    const handleRemove = () => {
        console.log(from, subject, to, cc);
        ...
        axios
            .post('/api/recruitment/candidates/remove', {
                ...ticket,
                candidate_id: data.candidate_id,
                message: { html: parse_html, text: parse_text, from, to, subject, cc: arrayCc },
            })
            .then((res) => {
