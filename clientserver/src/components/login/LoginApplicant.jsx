...
function Login() {
    ...
    const onSubmit = async (data) => {
        await axios
            .post(server + '/login/applicant', data)
            .then((res) => {
                setAlert({
    ...
    return (
        <div>
            <Nav />
            <div className="container login-form-applicant" style={{ maxWidth: '500px' }}>
                <form onSubmit={handleSubmit(onSubmit)}>
                    <h1>Login</h1>
                    <h5>Aplicants only</h5>
                    <Alert alert={alert} />
                    <Input
                        text="Email"
                        err={errors}
                ...
                <div className="mb-3 login-link">
                    Create an account?{' '}
                    <a href={postid ? 'register_applicant?postid=' + postid : 'register_applicant'}>
                        Register Here
                    </a>
                </div>
            </div>
